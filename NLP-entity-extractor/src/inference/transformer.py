...
class TransformerTokenClassifierExtractor:
    def __init__(self, model_path: Path, model_id: str = MODEL_ID) -> None:
    ...
    @classmethod
    def from_pretrained(cls, model_path: str | Path) -> "TransformerTokenClassifierExtractor":
        ...
        return cls(model_path, model_id=model_id)
    ...
    def _predict_entities(self, text: str) -> Iterable[EntitySpan]:
        for chunk_text, chunk_start in self._iter_chunks(text):
            for item in self.pipeline(chunk_text):
